...
#!/usr/bin/env python3
"""
📡 Pump.fun Tracker - RPC + CoinGecko Edition

• Reads token list from CSV (generated by token fetcher)
• Uses only Solana RPC + CoinGecko for all 13 metrics
• No Moralis dependency during tracking
"""
...
class RPCTracker:
        ...
        try:
            async with self.session.get(COINGECKO_URL) as response:
                if response.status == 200:
                    ...
                    self.log.info(
                        f"✅ CoinGecko connection successful! SOL price: ${sol_price}")
                else:
    ...
    async def _start_tracking_tokens(self, tokens: List[Dict]):
        ...
        self.log.info(f"🚀 Starting tracking for {len(tokens)} tokens...")
        ...
        for token in tokens:
            try:
                ...
                if remaining_minutes <= 0:
                    self.log.warning(
                        f"⚠️  Skipping {name} ({mint[:8]}) - tracking period expired")
                    continue

                # REMOVED ACTIVITY FILTER - Track all tokens within time window
                self.log.info(f"✅ TRACKING: {name} ({mint[:8]}) - {age_minutes:.1f}min old, {remaining_minutes:.1f}min remaining")
        ...
        self.log.info(
            f"✅ Started tracking {active_count} active tokens out of {len(tokens)} total")
    ...
    async def _sampler(self, t: SamplerTask):
        ...
        self.log.info(
            f"🚀 Tracking {t.name} until {end_time.strftime('%H:%M:%S')}")
        ...
        while datetime.now(timezone.utc) < end_time:
            try:
                ...
                if sample_count % 6 == 0:
                    ...
                    self.log.info(
                        f"📊 {t.name} | {elapsed/60:.1f}min elapsed, {remaining/60:.1f}min remaining")
                    self.log.info(
                    ...
                    if row['progress'] >= 100:
                        self.log.info(
                            f"🎉 {t.name} has migrated! Stopping tracking.")
                        break
        ...
        self.log.info(
            f"✅ Completed tracking {t.name} ({sample_count} samples)")

        if t.mint in self.active_tasks:
            del self.active_tasks[t.mint]

    # ── COLLECT METRICS USING ONLY RPC + COINGECKO ────────────────────────
    async def _collect_metrics(self, t: SamplerTask) -> Dict:
        ...
        try:
            ...
            if len(transactions) > 0:
                self.log.info(
                    f"🎯 {t.name[:20]} | Found {len(transactions)} transactions!")
                for tx in transactions[:3]:
                    self.log.info(
                        f"   📊 {tx.signer[:8]} | {tx.amount_sol:.4f} SOL | {'BUY' if tx.is_buy else 'SELL'}")
            ...
            if progress > 0 or len(transactions) > 0 or metrics["unique_buyers"] > 0:
                self.log.info(
                    f"✅ {t.name[:20]} | Progress: {progress:.1f}% | R: {R:.4f} | Buyers: {metrics['unique_buyers']} | Price: ${price_usd:.8f}")
    ...
    async def _get_token_price_rpc(self, mint: str) -> Dict:
            ...
            async with self.session.get(url, timeout=5) as response:
                if response.status == 200:
                    ...
                    if pairs and len(pairs) > 0:
                        ...
                        if price_usd > 0:
                            self.log.debug(
                                f"✅ DexScreener price for {mint[:8]}: ${price_usd}")
        ...
        try:
            ...
            async with self.session.get(url, timeout=5) as response:
                if response.status == 200:
                    ...
                    if price_usd > 0:
                        self.log.debug(
                            f"✅ Geckoterminal price for {mint[:8]}: ${price_usd}")
                        return {"price_usd": price_usd}
        except Exception as e:
        ...
        try:
            ...
            if recent_trades:
                ...
                if avg_price > 0:
                    self.log.debug(
                        f"✅ Calculated price for {mint[:8]}: ${avg_price}")
                    return {"price_usd": avg_price}
        except Exception as e:
            self.log.debug(f"Trade calculation error for {mint[:8]}: {e}")

        self.log.warning(f"❌ No price data found for {mint[:8]}")
    ...
    async def _get_bonding_curve_state_rpc(self, mint: str) -> Dict:
        ...
        try:
            ...
            self.log.info(
                f"📊 {mint[:8]} Progress: {progress:.1f}% | Buy: {total_buy_volume:.3f} SOL | Sell: {total_sell_volume:.3f} SOL | Net: {net_volume:.3f}")
        ...
        try:
            ...
            for signature in token_signatures[:20]: # Check more transactions
                ...
                if tx_data:
                    ...
                    self.log.debug(
                        f"✅ Valid transaction: {tx_data.signer[:8]} | {tx_data.amount_sol:.4f} SOL | Buy: {tx_data.is_buy}")
    ...
    def _detect_bot_activity(self, timestamps: List[float], volumes: List[float]) -> bool:
        """Detect bot-like trading patterns: ≥4 sub-0.1 SOL buys within 0.2s"""
        ...
        for i in range(len(trades) - 3):
            ...
            if time_span <= 0.2:
                ...
                if len(small_trades) >= 4:
                    self.log.info(
                        f"🤖 Bot-like activity detected: 4 trades < 0.1 SOL within {time_span:.3f}s")
                    return True
        ...
        try:
            ...
            for signature in signatures:
                ...
                if tx_data and self._has_burn_instruction(tx_data):
                    self.log.info(
                        f"🔥 LP burn detected for {mint[:8]} in {signature[:8]}")
                    return True
    ...
    async def _monitor_tasks(self):
        ...
        self.log.info("✅ All tracking completed!")

    async def _heartbeat(self):
        """Heartbeat logging"""
        while True:
            self.log.info(
                f"⏰ heartbeat | active samplers = {len(self.active_tasks)}")
    ...
    async def _check_token_activity(self, mint: str) -> bool:
        ...
        try:
            ...
            if len(signatures) < 3:
                self.log.debug(
                    f"❌ {mint[:8]} has only {len(signatures)} signatures - inactive")
                return False
            ...
            if pump_transactions == 0:
                self.log.debug(
                    f"❌ {mint[:8]} has no pump.fun transactions - inactive")
                return False

            self.log.debug(
                f"✅ {mint[:8]} has {pump_transactions} pump transactions - active")
            return True
